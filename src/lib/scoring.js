...
const ESCALATION_WORDS = [
  ...
  "widen",
  "expand the conflict",
  "intercepts.*launched from",
  "in response to",
  "follows an.*strike",
  "cluster bomb",
  "ballistic missile.*wounds",
  "rocket fire kills",
  "planning to invade",
  "capture its",
  "marines.*arrive",
...
export function computeSeverityScore(text, eventType) {
  ...
  for (const tier of SEVERITY_TIERS) {
    ...
    if (matches) {
      ...
      return (
        Math.round(
          Math.min(10, tier.score + Math.min(boostCount * 0.3, 1.0)) * 10,
        ) / 10
      );
    }
  }
  ...
  const ageLabel =
    publishedHoursAgo < 2
      ...
      : publishedHoursAgo < 24
        ? "Recent"
        : `${Math.round(publishedHoursAgo / 24)}d old`;
...
export function enrichEvent(rawEvent) {
  ...
  const hoursAgo = published_at
    ? (Date.now() - new Date(published_at).getTime()) / 3_600_000
    : 0;
