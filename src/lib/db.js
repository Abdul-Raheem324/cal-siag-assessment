// ============================================
// DEDUPLICATION + ACTOR RELATIONS UPDATER
// ============================================

import { supabase } from "./supabaseClient.js"


// Check if event already exists (by URL or very similar text)
...
// Increase corroboration count on existing similar events
export async function checkAndCorroborate(claimText, country, eventType) {
  ...
  if (data && data.length > 0) {
    ...
    // Bump corroboration count and recalculate confidence
    ...
    return true // was a corroboration, skip insert
  }
  return false
}
...
export async function upsertActor(actorName, country) {
  ...
  if (existing && existing.length > 0) {
    await supabase
      ...
      .eq('name', actorName)
    // console.log("updated called inside upsertActor");
  } else {
    await supabase
      .from('actors')
      .insert({
    ...
    //   console.log("insert called inside upsertActor");
  }
}

// Update actor_relations table (powers D3 graph - Idea 4)
export async function updateActorRelation(actor1, actor2, interactionType, domain, eventId) {
  ...
  if (existing && existing.length > 0) {
    await supabase
      ...
      .eq('id', existing[0].id)
    // console.log("updated called inside updateActorRelation");
  } else {
    await supabase
      .from('actor_relations')
      .insert({
...
// Master insert function - call this for every enriched event
export async function insertEvent(enrichedEvent) {
  // 1. Check duplicate
  const dup = await isDuplicate(enrichedEvent.source_url, enrichedEvent.claim_text)
  if (dup) {
  ...
  if (corroborated) {
    console.log(`🔗 Corroboration updated for: ${enrichedEvent.claim_text.slice(0, 60)}`)
    return null
  }
