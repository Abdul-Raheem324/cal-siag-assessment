...
const TIER = {
  1: {
    ...
    border: "rgba(46,204,142,0.3)",
  },
  2: {
    label: "Tier 2",
    desc: "Guardian, Al Jazeera",
    color: "#4d94ff",
    bg: "rgba(77,148,255,0.12)",
    border: "rgba(77,148,255,0.3)",
  },
  3: {
    label: "Tier 3",
    desc: "Regional media",
...
export default function SourcesView({ events }) {
  ...
  (events || []).forEach((ev) => {
    ...
    if (ev.domain)
      sourceMap[key].domains[ev.domain] =
        (sourceMap[key].domains[ev.domain] || 0) + 1;
  });
  ...
  const sources = Object.values(sourceMap)
    ...
    .map((s) => ({
      ...
      topDomain:
        Object.entries(s.domains).sort((a, b) => b[1] - a[1])[0]?.[0] || "—",
    }));

  const totalByTier = { 1: 0, 2: 0, 3: 0, 4: 0 };
  sources.forEach((s) => {
    if (totalByTier[s.tier] !== undefined) totalByTier[s.tier] += s.count;
  });
        ...
        {[1, 2, 3].map((tier, i) => {
          ...
          return (
            <div
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.boxShadow = `0 0 0 1px ${ti.border}, 0 4px 20px rgba(0,0,0,0.4)`;
              }}
              onMouseLeave={(e) => {
            ...
            >
              {/* Corner glow */}
              <div
                className="absolute top-0 right-0 w-20 h-20 opacity-20 group-hover:opacity-40 transition-opacity pointer-events-none"
                style={{
                  background: `radial-gradient(circle, ${ti.color} 0%, transparent 70%)`,
                }}
              />

              <div
                className="inline-flex items-center px-2.5 py-1 rounded-md border text-xs font-semibold mb-3"
                style={{
                  background: ti.bg,
                  color: ti.color,
                  borderColor: ti.border,
      ...
      >
        ...
        >
          ...
          <p className="text-xs mt-0.5" style={{ color: "#627a9e" }}>
            {sources.length} sources · sorted by event count
          </p>
        </div>

        {/* Header row */}
        <div
          className="hidden md:grid grid-cols-12 gap-3 px-4 py-2.5 border-b
            ...
            return (
              ...
              >
                <div className="md:col-span-4 flex items-center gap-2.5">
                  <span
                    className="w-6 h-6 rounded-md flex items-center justify-center text-[10px] font-bold shrink-0 border"
                    style={{
                      background: "#1f2840",
                      borderColor: "#2e3d58",
                      color: "#5d7599",
                ...
                <div className="md:col-span-2 flex items-center gap-1.5">
                  ...
                  <span
                    className="text-xs hidden lg:block capitalize"
                    style={{ color: "#627a9e" }}
                  >
                    {s.type}
                  </span>
                ...
                <div
                  className="md:col-span-1 text-right font-mono text-sm font-bold"
                  style={{ color: "#f1f5ff" }}
                >
                  {s.count}
                </div>
