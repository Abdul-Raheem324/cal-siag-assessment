...
const sevColor = (s) =>
  s >= 8
    ...
    : s >= 6
      ? "var(--color-threat-orange)"
      : s >= 4
        ? "var(--color-threat-amber)"
        : "var(--color-dim)";
const confColor = (c) =>
  c >= 7
    ? "var(--color-threat-green)"
    : c >= 5
      ? "var(--color-threat-amber)"
      : "var(--color-threat-red)";
      ...
      >
        ...
        >
          <div>
            <div className="flex flex-wrap gap-1.5 mb-1.5">
              <Tag color={sevColor(event.severity_score)}>
                SEV {event.severity_score?.toFixed(1)}
              </Tag>
              <Tag color="var(--color-domain-political)">
                {(event.event_type || "other").replace("_", " ").toUpperCase()}
              </Tag>
              <Tag color="var(--color-dim)">{event.domain?.toUpperCase()}</Tag>
              {event.is_escalation_signal && (
                <Tag color="var(--color-threat-orange)">↑ ESCALATION</Tag>
              )}
              {event.verified && (
                <Tag color="var(--color-threat-green)">✓ VERIFIED</Tag>
              )}
            </div>
        ...
        <div className="px-4 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field
              label="ACTION"
              value={event.actor_action?.toUpperCase() || "—"}
            />
            <Field
              label="LOCATION"
              value={`${event.country} · ${event.location_text || "—"}`}
            />
          </div>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2">
              <ConfBar
                label="SOURCE TIER"
                v={event.confidence_source_tier}
                max={9}
              />
              <ConfBar label="RECENCY" v={event.confidence_recency} max={10} />
              <ConfBar
                label="CORROBORATION"
                v={event.confidence_corroboration}
                max={9}
              />
              <ConfBar
                label="OVERALL"
                v={event.confidence_score}
                max={10}
                bold
              />
            </div>
            <p
              ...
              style={{ color: "var(--color-muted)" }}
            >
              {event.source_name} · TIER {event.source_tier} ·{" "}
              {event.corroboration_count} SOURCE(S)
            </p>
          </div>
        ...
        <div
          className="px-4 py-1.5 text-[9px] tracking-widest"
          style={{
...
function Tag({ color, children }) {
  return (
    <span
      className="text-[9px] px-1.5 py-px tracking-wide rounded-sm border"
      style={{
        color,
...
function Field({ label, value }) {
  return (
    <div>
      <p className="text-[9px] mb-0.5" style={{ color: "var(--color-muted)" }}>
        {label}
      </p>
      <p className="text-[11px]" style={{ color: "var(--color-mid)" }}>
        {value}
      </p>
    </div>
  );
...
function ConfBar({ label, v, max, bold }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span
        className="text-[9px] w-28 shrink-0"
        style={{ color: "var(--color-muted)" }}
      >
        {label}
      </span>
      ...
      <span
        className="text-[9px] tabular-nums min-w-[28px] text-right"
        style={{
          color: bold ? "var(--color-bright)" : "var(--color-dim)",
