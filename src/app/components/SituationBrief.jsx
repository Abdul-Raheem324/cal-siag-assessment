...
export default function SituationBrief({ stats, events }) {
  ...
  useEffect(() => {
    ...
    setBrief({
      ...
      lines: [
        ...
        topEvent
          ? `Highest: ${topEvent.event_type?.replace("_", " ")} in ${topEvent.country} (${topEvent.severity_score}/10).`
  ...
  return (
    ...
    >
      <div
        className="flex justify-between items-center px-3 py-1.5 shrink-0"
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2.5 space-y-3">
        {!brief ? (
          ...
          <>
            {brief.ai_text ? (
              <p
                className="text-[10px] leading-relaxed pl-2"
                style={{
                  color: "var(--color-mid)",
                  borderLeft: "2px solid rgb(59 130 246 / 0.3)",
                }}
              >
                {brief.ai_text}
              </p>
