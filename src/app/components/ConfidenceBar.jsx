...
const TIER_LABEL = {
  1: "Tier 1 (Reuters/AP)",
  2: "Tier 2 (Guardian/AJE)",
  3: "Tier 3 (Regional)",
  4: "Tier 4 (Social/Blog)",
};
const RECENCY_LABEL = (r) =>
  r >= 2.5 ? "Recent (<6h)" : r >= 1.5 ? "Same day" : "Older (24h+)";
...
export default function ConfidenceBar({ event }) {
  ...
  return (
    ...
    >
      <span
        className="font-mono text-[10px] font-medium w-6 shrink-0"
        style={{ color }}
      >
        {score.toFixed(1)}
      </span>
      ...
      {show && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 bg-bg-3 border border-line-hi rounded-sm shadow-xl z-50 p-3 pointer-events-none">
          ...
          {[
            ...
            {
              label: "Recency (30%)",
              val: RECENCY_LABEL(rec),
              score: rec,
              color: "#f5a623",
            },
            {
              label: "Corroboration (20%)",
              val: `${event.corroboration_count || 0} source(s)`,
              score: corr,
              color: "#22c55e",
            },
          ].map((row) => (
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="font-mono text-[8px] text-ink-3 mb-0.5">
                  {row.label}
                </div>
                <div className="font-mono text-[9px] text-ink-1 truncate">
                  {row.val}
                </div>
              </div>
              <span
          ...
          <div className="font-mono text-[8px] text-ink-3 mt-2 pt-1.5 border-t border-line">
            via{" "}
            <span className="text-ink-1">{event.source_name || "Unknown"}</span>
          </div>
        </div>
      )}
    </div>
