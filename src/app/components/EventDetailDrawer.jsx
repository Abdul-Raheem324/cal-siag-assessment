...
export default function EventDetailDrawer({ event, onClose }) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className="fixed right-0 top-0 h-full w-[420px] bg-bg-2 border-l border-line-hi z-50 flex flex-col overflow-y-auto animate-[slideIn_0.18s_ease-out]"
      ...
      >
        ...
        {/* Header */}
        <div
          className="flex items-center justify-between px-4 py-3 bg-bg-3 border-b shrink-0"
          style={{ borderColor: dc + "55" }}
        >
        ...
        <div className="flex items-center bg-bg-3 border-b border-line shrink-0">
          <ScoreBlock
            label="SEVERITY"
            value={`${(event.severity_score || 0).toFixed(1)}/10`}
            color={sc}
          />
          <div className="w-px h-8 bg-line" />
          <div className="flex flex-col items-center gap-1.5 flex-1 py-3">
            <span className="font-mono text-[8px] tracking-widest text-ink-3">
              CONFIDENCE
            </span>
          ...
          <ScoreBlock
            ...
            value={event.corroboration_count || 0}
            color="#3b82f6"
          />
        </div>
        ...
        {/* Claim */}
        <Section label="CLAIM">
          <p className="text-[12px] text-ink-0 leading-relaxed">
            {event.claim_text}
          </p>
        </Section>
        ...
        {event.ai_summary && (
          <Section label="AI SUMMARY">
            <p className="text-[11px] text-ink-1 leading-relaxed italic">
              {event.ai_summary}
            </p>
          </Section>
        )}
        ...
        <Section label="ACTORS">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {event.actor_action && (
              <span className="text-[10px] text-ink-2 italic">
                {event.actor_action}
              </span>
            )}
        ...
        <div className="grid grid-cols-2 border-b border-line">
          ...
          {event.longitude && (
            <Field label="LON" value={event.longitude} mono />
          )}
        </div>
        ...
        <Section label="SOURCE">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            <span className="text-[11px] text-ink-0 font-medium">
              {event.source_name}
            </span>
        ...
        <div className="grid grid-cols-2 border-b border-line">
          <Field
            label="EVENT TIME"
            value={fmt(event.event_datetime_utc)}
            mono
          />
          ...
          <Section label="TAGS">
            <div className="flex flex-wrap gap-1.5">
              {event.tags.map((t) => (
                <span
                  key={t}
                  className="font-mono text-[9px] text-ink-2 bg-bg-4 border border-line px-2 py-0.5 rounded-sm"
                >
                  {t}
                </span>
              ))}
        ...
        <div className="flex flex-wrap gap-2 px-4 py-3">
          ...
          {event.contradiction_flag && (
            <Flag label="CONTRADICTION FLAGGED" color="#a855f7" />
          )}
          {event.related_event_id && (
            <Flag label="LINKED EVENT" color="#3b82f6" />
          )}
        </div>
      </div>
    </>
...
function Section({ label, children }) {
  return (
    <div className="px-4 py-3 border-b border-line">
      <div className="font-mono text-[8px] tracking-widest text-ink-3 mb-2">
        {label}
      </div>
      {children}
    </div>
...
function Field({ label, value, mono }) {
  return (
    <div className="px-4 py-2.5 border-r border-b border-line last:border-r-0 [&:nth-child(even)]:border-r-0">
      <div className="font-mono text-[8px] tracking-widest text-ink-3 mb-1">
        {label}
      </div>
      <div
        className={`text-[11px] text-ink-1 ${mono ? "font-mono text-[10px]" : ""}`}
      >
        {String(value)}
      </div>
    </div>
