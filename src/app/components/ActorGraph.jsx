...
export default function ActorGraph({ actors = [], relations = [], onSelect, selected }) {
  const svgRef   = useRef(null);
  const wrapRef  = useRef(null);
  const simRef   = useRef(null);

  /*
   * tip = { actor, nodeX, nodeY, nodeR }
   * nodeX/Y are the D3 node positions in the current transform space.
   * We convert those to screen coords via the SVG element's transform.
   */
  ...
  useEffect(() => {
    ...
    import('d3').then(d3 => {
      ...
      const links = (relations || [])
        ...
        .map(r => ({
          source: r.actor_1, target: r.actor_2,
      ...
      node.append('circle')
        ...
        .attr('stroke', d => selected?.name === d.id ? (THREAT_COLOR[d.threat_level] || '#4d94ff') : 'none')
      ...
      node.append('text')
        ...
        .attr('font-weight', '500').attr('fill', '#e2ecff')
        .text(d => d.id);
  ...
  /*
   * Compute tooltip screen position from node's D3 coords + current zoom transform.
   * The tooltip renders inside the wrapRef div with position:absolute,
   * so coords must be relative to the wrapper, not the viewport.
   */
  const tipStyle = (() => {
      ...
      {tip && tipStyle && (
        <div
          ...
          style={{
            ...
            borderRadius: 12,
            padding:      '10px 12px',
            boxShadow:    '0 8px 32px rgba(0,0,0,0.7), 0 0 0 1px rgba(77,148,255,0.1)',
          }}
        >
          {/* Actor name */}
          <p style={{ fontWeight: 700, fontSize: 13, color: '#f1f5ff', marginBottom: 3 }}>
            {tip.actor.name}
          </p>
          {tip.actor.full_name && (
            <p style={{
              fontSize: 10, color: '#627a9e', marginBottom: 8,
              paddingBottom: 7, borderBottom: '1px solid #2e3d58',
              lineHeight: 1.4,
            }}>
              {tip.actor.full_name}
            </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            {[
              ...
              <div key={k} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 11, color: '#adbedd', textTransform: 'capitalize' }}>{v}</span>
              </div>
            ))}
