        ...
        <div className="flex flex-col gap-1.5">
          {domains.map(([domain, count]) => (
            <BarRow
              ...
              rightLabel={
                totalEvents ? `${Math.round((count / totalEvents) * 100)}%` : ""
              }
            />
          ))}
        </div>
...
function BarRow({ icon, label, count, pct, color, rightLabel }) {
  return (
    <div className="flex items-center gap-2">
      {icon && (
        <span className="text-[10px] text-ink-2 w-3 text-center shrink-0">
          {icon}
        </span>
      )}
      <span
      ...
      <div className="flex-1 h-1 bg-bg-4 rounded-full overflow-hidden">
        <div
          ...
          style={{ width: `${pct * 100}%`, background: color + "aa" }}
        />
      </div>
      <span className="font-mono text-[10px] text-ink-1 w-6 text-right shrink-0">
        {count}
      </span>
      {rightLabel && (
        <span className="font-mono text-[8px] text-ink-3 w-7 text-right shrink-0">
          {rightLabel}
        </span>
      )}
    </div>
