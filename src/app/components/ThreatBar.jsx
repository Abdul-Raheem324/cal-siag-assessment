...
const THREAT_CONFIG = {
  ...
  YELLOW: {
    label: "ELEVATED",
    color: "var(--color-threat-amber)",
    bgClass: "bg-yellow-950/40",
    blip: false,
  },
  ORANGE: {
    label: "HIGH",
    color: "var(--color-threat-orange)",
    bgClass: "bg-orange-950/40",
    blip: true,
  },
  RED: {
    label: "CRITICAL",
    color: "var(--color-threat-red)",
...
export default function ThreatBar({
  ...
  return (
    ...
    >
      {/* Brand */}
      <div
        className="flex items-center gap-2 px-4 h-full min-w-[200px]"
        style={{ borderRight: "1px solid var(--color-border)" }}
      >
        <span
          className="text-sm font-bold"
          style={{ color: "var(--color-threat-red)" }}
        >
          ▲
        </span>
        <span
          className="font-semibold tracking-[3px] text-[11px]"
      ...
      {/* Threat level */}
      <div
        className={`flex items-center gap-2.5 px-5 h-full ${cfg.bgClass}`}
        style={{ borderRight: "1px solid var(--color-border)" }}
      >
        <span
        ...
        <span className="font-bold text-[12px]" style={{ color: cfg.color }}>
          {cfg.label}
        </span>
      </div>
  ...
  return (
    <div className="flex flex-col gap-0.5 leading-none">
      ...
      <span
        ...
        style={{
          color: warn ? "var(--color-threat-orange)" : "var(--color-dim)",
        }}
      >
        {value}
      </span>
