...
const STAT_CARDS = (stats) => [
  ...
  },
  {
    ...
    sub: "Flagged high-urgency",
    icon: "⚡",
    accentColor: "#ffaa3b",
  },
  {
    label: "Avg Severity",
    value: stats?.avgSeverity ? `${stats.avgSeverity}/10` : "—",
    sub: "Across all events",
    icon: "📊",
    accentColor: stats?.avgSeverity >= 6 ? "#ff5370" : "#2ecc8e",
  },
  {
    label: "Avg Confidence",
    value: stats?.avgConfidence ? `${stats.avgConfidence}/10` : "—",
    sub: "Source reliability",
    icon: "🎯",
    accentColor: "#b880ff",
  },
];
...
const ChartTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div
      className="border rounded-xl px-3 py-2.5 text-xs"
      style={{
        background: "#1f2840",
        borderColor: "#2e3d58",
    ...
    >
      <p className="mb-2 font-medium" style={{ color: "#627a9e" }}>
        {label}
      </p>
      {payload.map((p) => (
        <div key={p.name} className="flex justify-between gap-5 mb-1 last:mb-0">
          <span style={{ color: "#adbedd" }} className="capitalize">
            {p.name}
          </span>
...
export default function OverviewView({ stats }) {
  ...
  const domainData = Object.entries(domainCounts)
    ...
    .map(([name, value]) => ({
      name,
      value,
      color: DOMAIN_COLORS[name] || "#5d7599",
    }));
    ...
    <div className="space-y-5">
      ...
      <SectionCard>
        <CardHeader
          ...
          subtitle="14-day event severity and volume"
        />
        {timeline.length > 0 ? (
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Tooltip
                  ...
                  cursor={{ stroke: "#2e3d58", strokeWidth: 1 }}
                />
                <Area
                  yAxisId="cnt"
                  type="monotone"
        ...
        <div
          className="flex items-center gap-5 mt-3 pt-3 border-t"
          style={{ borderColor: "#2e3d58" }}
        >
          {[
            { color: "#ff5370", label: "Avg Severity" },
            { color: "#4d94ff", label: "Event Count" },
          ].map((l) => (
            <div
              key={l.label}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <SectionCard>
          <CardHeader
            title="Event Types"
            subtitle="Top categories by frequency"
          />
            ...
            {Object.entries(stats?.typeCounts || {})
              ...
              .map(([type, count], i) => {
                ...
                return (
                  ...
                  >
                    <span
                      className="text-xs capitalize w-32 shrink-0 truncate transition-colors"
                      style={{ color: "#adbedd" }}
                    >
                      {type.replace(/_/g, " ")}
                    </span>
