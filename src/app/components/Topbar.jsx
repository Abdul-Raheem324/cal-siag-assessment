...
export default function TopBar({ stats }) {
  ...
  useEffect(() => {
    const tick = () => {
      const now = new Date();
      setUtc(now.toUTCString().replace(/^.*?, /, '').replace(' GMT', ' UTC'));
    };
    tick();
  ...
  return (
    <header className="flex items-center h-11 px-4 bg-bg-1 border-b border-line shrink-0 gap-4 z-50">
      ...
      <div className="flex items-center gap-2.5 shrink-0 font-mono">
        ...
        <span className="text-[13px] font-semibold tracking-[0.15em] text-ink-0">SAIG</span>
        <span className="text-ink-3 text-xs">/</span>
        <span className="text-[9px] tracking-[0.12em] text-ink-2">OSINT CONFLICT MONITOR</span>
      </div>
      ...
      <div className="flex items-center gap-1 flex-1 justify-center">
        ...
        <StatChip label="ESC SIGNALS"  value={stats?.escalationCount  ?? '—'} color="text-red" />
        <div className="h-4 w-px bg-line" />
        <StatChip label="AVG SEVERITY" value={stats?.avgSeverity      ?? '—'} color="text-amber" />
        <div className="h-4 w-px bg-line" />
        <StatChip label="AVG CONF"     value={stats?.avgConfidence    ?? '—'} color="text-green" />
      </div>

      {/* ── Threat ── */}
      <div className={`flex items-center gap-1.5 font-mono text-[9px] font-semibold tracking-[0.12em] px-3 py-1 border rounded-sm shrink-0 ${THREAT_STYLES[level]}`}>
        <span className={`w-1.5 h-1.5 rounded-full animate-pulse ${THREAT_DOT[level]}`} />
        THREAT: {level}
      </div>
      ...
      {/* ── Clock ── */}
      <span className="font-mono text-[10px] text-ink-2 tracking-wide shrink-0">{utc}</span>
    </header>
  );
}
...
function StatChip({ label, value, color = 'text-ink-0' }) {
  return (
    <div className="flex flex-col items-center gap-0.5 px-3">
      <span className={`font-mono text-base font-medium leading-none ${color}`}>{value}</span>
      <span className="font-mono text-[8px] tracking-[0.1em] text-ink-3">{label}</span>
    </div>
  );
}