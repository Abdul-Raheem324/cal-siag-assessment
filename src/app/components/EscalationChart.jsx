...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-bg-3 border border-line-hi rounded-sm px-3 py-2 shadow-xl">
      <div className="font-mono text-[9px] text-ink-2 mb-2 pb-1.5 border-b border-line">
        {d?.date}
      </div>
      ...
      <div className="flex justify-between gap-4 font-mono text-[10px] mb-1">
        ...
        <span
          style={{
            color: (d?.avgSeverity || 0) >= 6.5 ? "#e84040" : "#f5a623",
          }}
        >
          {d?.avgSeverity}
        </span>
...
export default function EscalationChart({ timeline = [] }) {
  ...
  const maxEscDay = timeline.reduce(
    (m, d) => (d.escalations > (m?.escalations || 0) ? d : m),
    null,
  );
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-3 py-2 border-b border-line shrink-0">
        <span className="font-mono text-[9px] font-semibold tracking-widest text-ink-1">
          ESCALATION TIMELINE
        </span>
        <span className="font-mono text-[8px] text-ink-3">
          14-day · ⚡ = escalation spike
        </span>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Tooltip
              ...
              cursor={{ stroke: "#283350", strokeWidth: 1 }}
            />
