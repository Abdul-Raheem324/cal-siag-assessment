...
function LeafletMap({ clusters, onClusterClick, selectedCluster }) {
  ...
  return (
    /*
     * Height strategy:
     * - On mobile  : fixed 420px  → never collapses, always visible
     * - On desktop : fixed 560px  → comfortable viewing area
     * DO NOT use h-full / 100% here — parent may not have a defined height,
     * which causes Leaflet to get 0px and render blank.
     */
    <div
      ref={containerRef}
      style={{
...
export default function MapView({ events = [] }) {
  ...
  const clusterEvList = selectedCluster
    ? [...selectedCluster.events].sort((a, b) => b.severity_score - a.severity_score)
    : [];

  const DOMAINS  = ['military', 'political', 'humanitarian', 'cyber', 'economic'];
  const SEV_OPTS = [{ label: 'All', val: null }, { label: '7+', val: 7 }, { label: '5+', val: 5 }];

  return (
    <div className="flex flex-col gap-4">

      {/* Filter bar */}
      <div className="rounded-xl border p-3 flex flex-wrap gap-2 items-center animate-fade-in-up"
        style={{ background: '#171e2c', borderColor: '#2e3d58' }}>

        <span className="text-xs font-semibold" style={{ color: '#627a9e' }}>Domain:</span>
        <FilterBtn active={!domainFilter} onClick={() => setDomainFilter(null)} color="#4d94ff">All</FilterBtn>
        {DOMAINS.map(d => (
          <FilterBtn key={d} active={domainFilter === d} onClick={() => setDomainFilter(domainFilter === d ? null : d)} color={DOMAIN_COLOR[d]}>
            {d}
          </FilterBtn>
        ))}

        <div className="w-px h-5 hidden sm:block" style={{ background: '#2e3d58' }} />

        <span className="text-xs font-semibold" style={{ color: '#627a9e' }}>Severity:</span>
        {SEV_OPTS.map(s => (
          <FilterBtn key={s.label} active={sevFilter === s.val} onClick={() => setSevFilter(s.val)} color="#ff5370">
            {s.label}
          </FilterBtn>
        ))}
      ...
      {/* Map card */}
      <div className="rounded-xl border overflow-hidden relative animate-fade-in-up"
        style={{ background: '#0d1117', borderColor: '#2e3d58' }}>

        {/* Legend — absolute over map */}
        <div className="absolute top-3 left-3 z-[400] rounded-lg border p-2.5"
          style={{ background: 'rgba(17,24,39,0.93)', borderColor: '#2e3d58', backdropFilter: 'blur(8px)' }}>
          <p className="text-[9px] uppercase tracking-widest font-semibold mb-2" style={{ color: '#627a9e' }}>
            Severity
          </p>
          {[
            ...
            <div key={l.label} className="flex items-center gap-2 mb-1 last:mb-0">
              ...
              <span className="text-[10px]" style={{ color: '#adbedd' }}>{l.label}</span>
            </div>
          ))}
          <p className="text-[9px] mt-2 pt-2 border-t" style={{ color: '#627a9e', borderColor: '#2e3d58' }}>
            Number = event count
          </p>
        </div>
        ...
        {/* Click hint */}
        {!selectedCluster && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-[400] px-3 py-1.5 rounded-lg border text-xs pointer-events-none"
            style={{ background: 'rgba(17,24,39,0.9)', borderColor: '#2e3d58', color: '#adbedd', backdropFilter: 'blur(8px)', whiteSpace: 'nowrap' }}>
      ...
      <div className="animate-fade-in-up">
        {selectedCluster ? (
          <div className="rounded-xl border overflow-hidden"
            style={{ background: '#171e2c', borderColor: '#2e3d58', boxShadow: '0 4px 24px rgba(0,0,0,0.4)' }}>

            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b"
              style={{ background: '#1f2840', borderColor: '#2e3d58' }}>
              <div>
                <h3 className="text-sm font-bold" style={{ color: '#f1f5ff' }}>{selectedCluster.country}</h3>
                <p className="text-xs mt-0.5" style={{ color: '#627a9e' }}>
                  {clusterEvList.length} event{clusterEvList.length !== 1 ? 's' : ''} · max severity {selectedCluster.maxSev.toFixed(1)}
                </p>
              </div>
              <button onClick={() => setSelectedCluster(null)}
                ...
                onMouseLeave={e => { e.currentTarget.style.color = '#627a9e'; }}>
                ✕
              </button>
            </div>

            {/* Severity tally */}
            <div className="px-4 py-3 border-b grid grid-cols-4 gap-2" style={{ borderColor: '#2e3d58' }}>
              {[['Critical','#ff5370'],['High','#ffaa3b'],['Medium','#ffd166'],['Low','#2ecc8e']].map(([lbl, col]) => (
                <div key={lbl} className="text-center">
                  <p className="text-sm font-bold font-mono" style={{ color: col }}>
                    {clusterEvList.filter(e => sevLabel(e.severity_score) === lbl).length}
                  </p>
                  <p className="text-[9px] uppercase" style={{ color: '#627a9e' }}>{lbl}</p>
                </div>
              ))}
            </div>

            {/* Event list — grid on desktop, stacked on mobile */}
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x overflow-y-auto"
                ...
                >
                  <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                    <span className="text-[10px] font-bold font-mono px-1.5 py-0.5 rounded border"
                      style={{ color: sevColor(ev.severity_score), background: `${sevColor(ev.severity_score)}15`, borderColor: `${sevColor(ev.severity_score)}35` }}>
                    ...
                    {ev.event_type && ev.event_type !== 'other' && (
                      <span className="text-[10px] capitalize" style={{ color: '#627a9e' }}>
                        {ev.event_type.replace(/_/g, ' ')}
                      </span>
                    )}
                    <span className="ml-auto text-[10px]" style={{ color: '#627a9e' }}>
                      {timeAgo(ev.event_datetime_utc)}
                    </span>
                  </div>
                  <p className="text-xs leading-relaxed line-clamp-2 mb-1.5" style={{ color: '#c8d8f0' }}>
                    {ev.claim_text}
                  </p>
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    ...
                    {ev.source_url && (
                      <a href={ev.source_url} target="_blank" rel="noopener"
                        className="text-[10px]" style={{ color: '#4d94ff' }}
          ...
          <div className="rounded-xl border p-4" style={{ background: '#171e2c', borderColor: '#2e3d58' }}>
            <p className="text-xs font-semibold mb-3" style={{ color: '#adbedd' }}>Active Locations</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {[...clusters].sort((a, b) => b.maxSev - a.maxSev).map(c => (
                <button key={c.key}
                  onClick={() => setSelectedCluster(c)}
                  className="flex items-center justify-between px-3 py-2.5 rounded-lg border cursor-pointer transition-all text-left"
                  style={{ background: '#1f2840', borderColor: '#2e3d58' }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = sevColor(c.maxSev) + '60'; e.currentTarget.style.background = '#263147'; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = '#2e3d58'; e.currentTarget.style.background = '#1f2840'; }}
                >
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full shrink-0"
                      style={{ background: sevColor(c.maxSev), boxShadow: `0 0 6px ${sevColor(c.maxSev)}80` }} />
                    <span className="text-xs font-medium" style={{ color: '#f1f5ff' }}>{c.country}</span>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-xs font-mono font-bold" style={{ color: sevColor(c.maxSev) }}>
                      {c.maxSev.toFixed(1)}
                    </span>
                    <span className="text-[10px]" style={{ color: '#627a9e' }}>{c.events.length} ev</span>
                  </div>
                </button>
              ))}
            </div>
