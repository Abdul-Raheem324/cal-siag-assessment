...
export function useStats() {
  ...
  const load = useCallback(async () => {
    try { setData(await (await fetch('/api/stats')).json()); }
...
export function useActors() {
  ...
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    fetch('/api/actors').then(r => r.json()).then(j => setActors(j.actors || [])).finally(() => setLoading(false));
  }, []);
...
export function useRelations() {
  ...
  const [loading, setLoading]     = useState(true);
  useEffect(() => {
    fetch('/api/relations').then(r => r.json()).then(j => setRelations(j.relations || [])).finally(() => setLoading(false));
  }, []);
